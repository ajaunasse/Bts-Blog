...
class PostsController extends Controller{
	...
	**/
		...
		function blog()
	{
		$perPage = 3 ;
		$this->loadModel('Post') ;
		$conditions = array('online' => 1, 'type'=>'post', 'category_id !'=>3) ;
		$d['posts'] = $this->Post->find(array(
			'fields' 		=> 'Post.id,Post.name,SUBSTRING(Post.content,1,500) AS content, LENGTH(RTRIM(content)) as countWord, Post.created,Post.online,Post.type,Post.slug, user_id, Category.name as catname, Post.category_id as idcat',
			'conditions' 	=>$conditions,
			'order by'		=>'created DESC',
			'limit' 		=> ($perPage*($this->request->page-1)).','.$perPage,
			'join'		 => array('categories as Category'=>'Category.id=Post.category_id') 
